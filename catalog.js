(function(){
  ...
  document.addEventListener('DOMContentLoaded',async()=>{try{const d=await C.ready,links=d.categories.map(c=>`<a href="/shop.html?category=${encodeURIComponent(c.slug)}">${S.escape(c.name)}</a>`).join('');document.querySelectorAll('[data-category-nav],[data-category-footer]').forEach(e=>e.innerHTML=links);
    ...
    await C.libraryReady;const grid=document.getElementById('featuredGrid');if(grid)grid.innerHTML=d.products.filter(p=>p.featured).slice(0,4).map(C.card).join('')||'<p>No featured products yet. Explore the shop.</p>';
    const hero=document.querySelector('[data-catalog-hero]');if(hero)hero.innerHTML=d.products.filter(p=>p.featured&&p.cover_path).slice(0,3).map(p=>`<div class="cover hero-image-cover"><img src="${S.escape(S.safeURL(p.cover_path))}" alt="${S.escape(p.name)}" decoding="async" width="1086" height="1448"></div>`).join('');
  }catch{for(const id of['categoryGrid','featuredGrid']){const e=document.getElementById(id);if(e)e.innerHTML='<p class="notice">The catalog is temporarily unavailable. Please try again later.</p>';}}});
})();
